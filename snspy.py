...
__version__ = '3.7.1'
__author__ = 'Liao Xuefeng (askxuefeng@gmail.com), HONG Manzhi(a88185807@qq.com)'
...
class JsonDict(dict):
    ...
    def __getattr__(self, attr):
        try:
            ...
            raise AttributeError(r"'JsonDict' object has no attribute '{}'".format(attr))
...
def _encode_params(**kw):
    ...
    def _encode(l, k, v):
        if isinstance(v, str):
            l.append('{}={}'.format(k, quote(v.encode('utf-8'))))
        elif isinstance(v, collections.Iterable):
            for x in v:
                _encode(l, k, x)
        else:
            l.append('{}={}'.format(k, quote(str(v))))
    args = []
    for k, v in kw.items():
        _encode(args, k, v)
    return '&'.join(args)
...
def _encode_multipart(**kw):
    ...
    for k, v in kw.items():
        ...
        if hasattr(v, 'read'):
            ...
            content = v.read()
            data.append('Content-Disposition: form-data; name="{}"; filename="hidden"'.format(k))
            data.append('Content-Length: {}'.format(len(content)))
            data.append('Content-Type: {}\r\n'.format(_guess_content_type(filename)))
            data.append(content)
        else:
...
def _http(method, url, headers=None, **kw):
    ...
    http_url = '{}?{}'.format(url, params) if method == _HTTP_GET else url
    http_body = None if method == 'GET' else params
    logging.error('{}: {}'.format(method, http_url))
...
class SNSMixin(object):
    ...
    def _prepare_api(self, method, path, access_token, **kw):
        raise BaseException('Subclass must implement \'_prepare_api\' method.')
...
class SinaWeiboMixin(SNSMixin):
    def get_authorize_url(self, redirect_uri, **kw):
        ...
        return 'https://api.weibo.com/oauth2/authorize?{}'.format(_encode_params(
            client_id=self._client_id, response_type=response_type, redirect_uri=redirect, **kw))
    ...
    def _prepare_api(self, method, path, access_token, **kw):
        ...
        if '/remind/' in path:
            ...
            return method, 'https://rm.api.weibo.com/2/{}.json'.format(path), headers, kw
        if method == 'POST' and 'pic' in kw:
            # if 'pic' in parameter, set to UPLOAD mode:
            return 'UPLOAD', 'https://api.weibo.com/2/{}.json'.format(path), headers, kw
        return method, 'https://api.weibo.com/2/{}.json'.format(path), headers, kw

    def request_access_token(self, code, redirect_uri=None):

        # Return access token as a JsonDict: {"access_token":"your-access-token",
        # "expires":12345678,"uid":1234},
    ...
    def parse_signed_request(self, signed_request):
        ...
        logging.info('parse signed request: {}'.format(sr))
...
class QQMixin(SNSMixin):
    ...
    def get_authorize_url(self, redirect_uri='', **kw):
        ...
        if not redirect:
            raise APIError('21305', 'Parameter absent: redirect_uri', 'OAuth2 request')
        response_type = kw.pop('response_type', 'code')
        return 'https://graph.qq.com/oauth2.0/authorize?{}'.format(
            _encode_params(client_id=self._client_id, response_type=response_type,
                           redirect_uri=redirect, **kw))

    def _prepare_api(self, method, path, access_token, **kw):
        kw['access_token'] = access_token
        kw['oauth_consumer_key'] = self._client_id
        return method, 'https://graph.qq.com/{}'.format(path), None, kw

    def request_access_token(self, code, redirect_uri=None):
        # Return access token as a JsonDict: {"access_token":"your-access-token",
        # "expires":12345678,"uid":1234}, expires is represented using standard unix-epoch-time
        redirect = redirect_uri or self._redirect_uri
...
class APIClient:
    ...
    def refresh_token(self, refresh_token):
        req_str = '{}{}'.format(self.auth_url, 'access_token')
    ...
    def call_api(self, http_method, http_path, **kw):
        ...
        logging.debug('Call API: {}: {}'.format(method, the_url))
        try:
...
class _Executable(object):
    ...
    def __str__(self):
        return '_Executable ({} {})'.format(self._method, self._path)
...
class _Callable(object):
    ...
    def __getattr__(self, attr):
        ...
        name = '{}/{}'.format(self._name, attr)
        return _Callable(self._client, name)

    def __str__(self):
        return '_Callable ({})'.format(self._name)

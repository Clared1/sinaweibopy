...
def _http_upload(url, authorization=None, **kw):
    logging.info('MULTIPART POST {}'.format(url))
...
def _http_call(the_url, method, authorization, **kw):
    ...
    http_url = '{}?{}'.format(the_url, params) if method == _HTTP_GET else the_url
    http_body = None if method == _HTTP_GET else params
    req = Request(http_url, data=http_body)
    req.add_header('Accept-Encoding', 'gzip')
    if authorization:
        req.add_header('Authorization', 'OAuth2 {}'.format(authorization))
    if boundary:
        req.add_header('Content-Type', 'multipart/form-data; boundary={}'.format(boundary))
    try:
        resp = urlopen(req, timeout=5)
        body = resp.headers.get()
...
class HttpObject:
    ...
    def __getattr__(self, attr):
        def wrap(**kw):
            ...
            return _http_call('{}{}.json'.format(self.client.api_url, attr.replace('__', '/')),
...
class APIClient:
    ...
    def __init__(self, app_key, app_secret, redirect_uri=None,
        ...
        self.auth_url = 'https://{}/oauth2/'.format(domain)
        self.api_url = 'https://{}/{}/'.format(domain, version)
    ...
    def get_authorize_url(self, redirect_uri=None, **kw):
        ...
        return '{}{}?{}'.format(self.auth_url, 'authorize',
                                _encode_params(client_id=self.client_id,
                                               response_type=response_type,
                                               redirect_uri=redirect, **kw))

    def _parse_access_token(self, r):
        # new:return access token as a JsonDict: {"access_token":"your-access-token","expires_in":12345678,
        # "uid":1234}, expires_in is represented using standard unix-epoch-time
    ...
    def request_access_token(self, code, redirect_uri=None):
        ...
        if not redirect:
            raise APIError('21305', 'Parameter absent: redirect_uri', 'OAuth2 request')
        r = _http_post('{}{}'.format(self.auth_url, 'access_token'),
...
class _Executable:
    ...
    def __call__(self, **kw):
        ...
        if method == _HTTP_POST and 'pic' in kw:
            method = _HTTP_UPLOAD
        return _http_call('{}{}.json'.format(self._client.api_url, self._path), method, self._client.access_token, **kw)
